...
class StorageUpdateTest extends StorageBase
{
    ...
    public function UpdateProvider() {
        return [
            [1,'Sunhill\\ORM\\Tests\\Objects\\Dummy',function($storage) {
                $storage->dummyint = ['FROM'=>123,'TO'=>321];
            },'dummyint',321],                       // Wird ein einfaches Feld geändert?
            [5,'Sunhill\\ORM\\Tests\\Objects\\TestParent',function($storage) {
            ...
            [6,'Sunhill\\ORM\\Tests\\Objects\\TestChild',function($storage) {
                $storage->parentint  = ['FROM'=>123,'TO'=>999];
                $storage->parentchar = ['FROM'=>'ABC','TO'=>'OOO'];
            ...
            [7,'Sunhill\\ORM\\Tests\\Objects\\Passthru',function($storage) {
                $storage->parentint = ['FROM'=>123,'TO'=>'999'];
...
// Tagtests            
            [1,'Sunhill\\ORM\\Tests\\Objects\\Dummy',function($storage) { // Nur hinzufügen
                $storage->tags = ['FROM'=>[1,2],'TO'=>[1,2,3],
                                  'ADD'=>[3],'DELETE'=>[]];
            },'tags',[1,2,3]],                         
            [1,'Sunhill\\ORM\\Tests\\Objects\\Dummy',function($storage) { // Ein Elemnt löschen
                $storage->tags = ['FROM'=>[1,2],'TO'=>[1],
                                  'ADD'=>[],'DELETE'=>[2]];
            },'tags',[1]],                            
            [1,'Sunhill\\ORM\\Tests\\Objects\\Dummy',function($storage) { // Alle Elemente löschen
                $storage->tags = ['FROM'=>[1,2],'TO'=>[],
                                  'ADD'=>[],'DELETE'=>[1,2]];
            },'tags',[]],                            
            [1,'Sunhill\\ORM\\Tests\\Objects\\Dummy',function($storage) { // Kombiertes löschen und hinzufügen
                $storage->tags = ['FROM'=>[1,2],'TO'=>[1,3],
                                  'ADD'=>[3],'DELETE'=>[2]];
            },'tags',[1,3]],                           

// Objektarraytests
            [6,'Sunhill\\ORM\\Tests\\Objects\\TestChild',function($storage) { // Objekt hinzufügen
                $storage->parentoarray = ['FROM'=>[0=>1,1=>2],'TO'=>[0=>1,1=>2,2=>3],
                                          'ADD'=>[2=>3],'DELETE'=>[]];
            },'parentoarray',[1,2,3]],
            [6,'Sunhill\\ORM\\Tests\\Objects\\TestChild',function($storage) { // Objekt löschen
                $storage->parentoarray = ['FROM'=>[0=>1,1=>2],'TO'=>[0=>1],
                                          'ADD'=>[],'DELETE'=>[1=>2]];
            },'parentoarray',[1]],
            [6,'Sunhill\\ORM\\Tests\\Objects\\TestChild',function($storage) { // Alle Objekte löschen
                $storage->parentoarray = ['FROM'=>[0=>1,1=>2],'TO'=>[],
                                          'ADD'=>[],'DELETE'=>[0=>1,1=>2]];
            },'parentoarray',null],
            [6,'Sunhill\\ORM\\Tests\\Objects\\TestChild',function($storage) { // Kombiniertes hinzufügen und löschen
                $storage->parentoarray = ['FROM'=>[0=>1,1=>2],'TO'=>[0=>1,1=>3],
                                          'ADD'=>[1=>3],'DELETE'=>[1=>2]];
            },'parentoarray',[1,3]],
            [6,'Sunhill\\ORM\\Tests\\Objects\\TestChild',function($storage) { // Änderung in anderen Feldern
